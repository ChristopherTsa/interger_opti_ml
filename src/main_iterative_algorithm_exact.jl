...
include("merge.jl")
include("main_merge_exact.jl")
include("shift.jl")
using CSV
using DataFrames
using Dates

function main_iterative_exact()
    ...
    for dataSetName in ["iris", "seeds", "wine", "breast_cancer", "ecoli"]
        ...
        for D in 2:4
            ...
            testIterative_exact(X_train, Y_train, X_test, Y_test, D, classes, results, dataSetName; time_limit = time_limit, isMultivariate = false, isExact=false)
            println("\t\t\t- Iterative heuristic (FhS) with shifts")
            testIterative_exact(X_train, Y_train, X_test, Y_test, D, classes, results, dataSetName; time_limit = time_limit, isMultivariate = false, isExact=false, shiftSeparations=true)
            println("\t\t\t- Iterative exact (FeS)")
            testIterative_exact(X_train, Y_train, X_test, Y_test, D, classes, results, dataSetName; time_limit = time_limit, isMultivariate = false, isExact=true)
...
#            testIterative(X_train, Y_train, X_test, Y_test, D, classes, time_limit = time_limit, isMultivariate = true, isExact=true)
    ...
    CSV.write("results/main_iterative_exact_results_$(timestamp).csv", results)
end 

function testIterative_exact(X_train, Y_train, X_test, Y_test, D, classes, results, dataSetName; time_limit::Int=-1, isMultivariate::Bool = false, isExact::Bool=false, shiftSeparations::Bool=false)
