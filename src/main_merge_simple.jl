...
function main_merge_simple()
    ...
    for dataSetName in ["iris", "seeds", "wine", "breast_cancer_", "ecoli_"]
        ...
        for D in 2:4
            ...
            println("\t\tMultivarié")
            testMerge_simple(X_train, Y_train, X_test, Y_test, D, classes, results, dataSetName; time_limit = time_limit, isMultivariate = true)
        end
    end
    ...
    CSV.write("results/main_merge_simple_results_$(timestamp).csv", results)
end 

function testMerge_simple(X_train, Y_train, X_test, Y_test, D, classes, results, dataSetName; time_limit::Int=-1, isMultivariate::Bool = false)
